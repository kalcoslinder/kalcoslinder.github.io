...
var messages = [
                ...
                "*Good job.\nNow try to find a lightswitch?",
                "You found it!",
                "*Cool, now let's see if we can find a clue to what's hiding around here.",
                "Hope you aren't afraid of a little mouse!"
                ];
...
var drawLiquorDoor = function(liquorDoor){
    fill(0, 179, 255);
  rect(liquorDoor.pos.x,liquorDoor.pos.y,liquorDoor.width,liquorDoor.height);
  stroke(143, 67, 13);
  strokeWeight(1);
  fill(0, 0, 0);
    rect(liquorDoor.pos.x+liquorDoor.width/2-1,liquorDoor.pos.y,2,liquorDoor.height);
};
}
...
var drawHobo = function(x,y){
    //Head
    strokeWeight(3);
    fill(255, 255, 255);
    ellipse(x,y+10,50,50);
    //line(x,y+40,x,y+40);
    line(x,y+38,x-31,y+59);
    line(x,y+39,x+31,y+59);
    line(x,y+39,x,y+59);
...
interaction.prototype.drawPortrait = function(){
  if(this.portrait === "hobo"){
      drawHobo(335,75);
  }
  if(this.portrait === "stickman"){
    drawStickman(65,75);    
  }
};
interaction.prototype.draw = function(){
    ...
    if(this.t !== 0){
        ...
        if(this.portrait === "stickman"){
        rect(this.x-360+this.width,this.y,95,95);
        }else{
            rect(this.x+265,this.y,95,95);
        }
        ...
        //Draw portrait
                ...
                rect(this.x,this.y,this.width-95,this.height);
        }
        this.drawPortrait();
        //Draw message
...
door.prototype.draw = function() {
    ...
    if(bobX > this.pos.x && bobX < this.pos.x + this.width)
    {
        ...
        if(this.interaction.state === 0){
            shop = "closed";
            if(mouseIsPressed && this.hold === false && mouseX>this.pos.x-5&&mouseX<this.pos.x+5+this.width&&mouseY>this.pos.y-30&&mouseY<this.pos.y+this.height+10){
                this.interaction.state = 1;
                this.hold = true;
            }
        }
        if(this.interaction.state === 1 && !mouseIsPressed){
               this.interaction.state = 2;
        }
        if(this.interaction.state === 2){
            if((mouseIsPressed && this.hold === false)){
                this.hold = true;
    ...
    mailbox.interaction.portrait = "stickman";
    mailbox.interaction.messages = ["Let's see what we got.","Wait this isn't my \nmail."];
    ...
    aptDoor.interaction.portrait = "stickman";
    aptDoor.interaction.messages = ["Alright, I need to use\n my key to open\n this door."];
    aptDoor.interaction.options = [[]];
    var hoboDoor = new door();
    hoboDoor.pos.x = 205;
    hoboDoor.width = 30;
    hoboDoor.height = 30;
    hoboDoor.show = false;
    hoboDoor.interaction.portrait = "hobo";
    
    hoboDoor.interaction.messages=["Want to know something?","Sure.","Get me another  beer."];
    ...
    liquorDoor.interaction.portrait = "stickman";
    liquorDoor.interaction.messages=["I need a beer.","Doesn't look like I can afford \nanything though."];
    liquorDoor.interaction.options=[["[·] Beer - 10g","[ ] Gun - 100g"],[]];
...
draw = function() {
    ...
    //Stop bob smoothly
    if(!keyIsPressed && lrun === false && (destX<bobX+5)){
    ...
    if(!keyIsPressed && run === false && (destX>bobX-5)){
    ...
    if(shop==="closed" && run ===false){
        if((keyCode === RIGHT && keyIsPressed)||(mouseX>bobX+5 && mouseIsPressed && mouseDown)&&mouseY<ground){ 
        mouseDown = true;
        lrun = false;
        if(bob.state === "stop")
        {
    ...
    if(shop==="closed" && lrun === false){
        if((keyCode === LEFT && keyIsPressed)||(mouseX<bobX-5 && mouseIsPressed && mouseDown)&&mouseY<ground){ 
            run = false;
            if(bob.state === "stop")
            {
                bob.state = "start";
    ...
    if(keyIsPressed&&keyCode === UP){
        ...
        text("bobX: "+bobX,5,415);
        text("runl: "+lrun,5,435);
        
        text("run: " + run,5,455);
        text("destX: "+destX,5,475);
